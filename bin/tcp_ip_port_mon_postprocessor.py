...
def build_con_dict():
    ...
    while True:  # Reading loop
        ...
        try:
            ...
            logtime = datetime.now().astimezone().isoformat()
            eprint(logtime + " [PID " + str(os.getpid()) + "]" +
                   " Error: " + DEF_CONNECTION_FIFO + " closed?")
            if not GLOBAL_SHUTDOWN:  # prevent re-triggering
                os.kill(os.getpid(), signal.SIGINT)
            return
        except BaseException:
...
def build_conntrack_dict():
    ...
    global ct_con_dict, ct_con_dict_lock, ct_con_dict_evt, ct_syn_dict, ct_syn_dict_lock, ct_syn_dict_evt
    logtime = datetime.now().astimezone().isoformat()
    eprint(logtime + " [PID " + str(os.getpid()) + "]" +
    ...
    while(1):
        ...
        for row in conntrack:  # Parse conntrack output
            ...
            # So with DNAT a missmatch may happen i a port is allready in use, so that instead of [D], wrongly [C] is in connection objects as src_port.
            # [B] is resolved in TCP Module by making use of getsockopt(..., SO_ORIGINAL_DST, ...)
            # ctid_syn as synid [X]_[Y=B]+[Z=D]:
            # Only established and closed are of intressed to determine if a connection object must (should) be present at some time
            # Other fields:
            ...
            # or:
            #   0     1     2[T]    3[S]      ORiGgin: 4                  5[h]             6[D]         7[B]       RePLy:   8                  9[h]             10[A]             11[l]           12[C]            13        14
...
syn_dict_th = threading.Thread(target=build_syn_dict)
syn_dict_th.setDaemon(True)
con_dict_th = threading.Thread(target=build_con_dict)
con_dict_th.setDaemon(True)
ct_dict_th = threading.Thread(target=build_conntrack_dict)
ct_dict_th.setDaemon(True)
# Threads for generating JSON and cleaning up dictonarys
output_syn_scans_th = threading.Thread(target=output_syn_scans)
